      ...
      if (res.detections.length > 0) {
        ...
        if (detection.landmarks) {
          ...
          if (rightEyeLandmark - rightearLandmark < 0.02) {
            this.base64array.turnright.push(base64call);
            this.typeface = "Turn right";
            if (this.base64array.turnright.length > 10) {
              let poppedElement = this.base64array.turnright.shift();
            } else {
            }
          }
          if (leftearLandmark - leftEyeLandmark < 0.02) {
            this.base64array.turnleft.push(base64call);
            this.typeface = "Turn left";
            //console.log(this.typeface);
            if (this.base64array.turnleft.length > 10) {
              let poppedElement2 = this.base64array.turnleft.shift();
            }
          }

          if (detection.landmarks[1].y - detection.landmarks[5].y > 0.02) {
            this.base64array.lookdown.push(base64call);
            this.typeface = "Look down";
            //console.log(this.typeface);
            if (this.base64array.lookdown.length > 10) {
              let poppedElement2 = this.base64array.lookdown.shift();
            }
          }

          if (detection.landmarks[2].y - detection.landmarks[5].y < 0.02) {
            this.base64array.lookup.push(base64call);
            this.typeface = "Look up";
            //console.log(this.typeface);
            if (this.base64array.lookup.length > 10) {
              let poppedElement2 = this.base64array.lookup.shift();
            }
          }
        }
      }
